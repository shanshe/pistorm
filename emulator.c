...
#ifdef FAKE_CHIP
unsigned char fake_chip[2 * SIZE_MEGA];
#endif
...
unsigned int m68k_read_memory_8(unsigned int address) {
  ...
  if (address < 2 * SIZE_MEGA) {
    if (fake_chip[address] == 0) {
      fake_chip[address] = (unsigned int)read8((uint32_t)address);
    }
    return fake_chip[address];
  }
#endif
...
#ifdef FAKE_CHIP
  if (address < 2 * SIZE_MEGA) {
    if (*(uint16_t *)(&fake_chip[address]) == 0) {
      *(uint16_t *)(&fake_chip[address]) = read16((uint32_t)address);
    }
    return be16toh(*(uint16_t *)(&fake_chip[address]));
  }
#endif
...
#ifdef FAKE_CHIP
  if (address < 2 * SIZE_MEGA) {
    if (*(uint32_t *)(&fake_chip[address]) == 0) {
      *(uint32_t *)(&fake_chip[address]) = read32((uint32_t)address);
    }
    return be32toh(*(uint32_t *)(&fake_chip[address]));
  }
#endif
...
#ifdef FAKE_CHIP
  if (address < 2 * SIZE_MEGA) {
    fake_chip[address] = (uint8_t)value;
  }
#endif
...
#ifdef FAKE_CHIP
  if (address < 2 * SIZE_MEGA) {
    *(uint16_t *)(&fake_chip[address]) = htobe16((uint16_t)value);
  }
#endif
...
#ifdef FAKE_CHIP
  if (address < 2 * SIZE_MEGA) {
    *(uint32_t *)(&fake_chip[address]) = htobe32(value);
  }
#endif
